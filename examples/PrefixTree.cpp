...
/* problem: https://codeforces.com/contest/1980/problem/G */
...
void solve(){
    ...
    auto dfs = [&](auto&& self, int u, int p, int sum, int d)->void{
        parity[u] = d;
        trie[parity[u]].insert(sum);
        value[u] = sum;
        for (const auto& [v, w] : al[u]) {
            if (v != p) {
                self(self, v, u, sum ^ w, d ^ 1);
            }
        }
    };
    ...
    while (m --) {
        ...
        if (t == '^') {
            ...
            cin >> v >> x;
            int p = parity[v];
            trie[p].erase(value[v]);
            cout << max(trie[p].getMaxXor(value[v] ^ x), trie[p ^ 1].getMaxXor(value[v] ^ x ^ cnt)) << ' ';
            trie[p].insert(value[v]);
        }
    }
    cout << '\n';
}
